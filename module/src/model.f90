...
module model 
	...
	subroutine initparams
		integer i
		character*80 buffer
		open(1,file='../params/params.info')
	...
	subroutine dosimulate(pop)
		...
		pp = 1
		open(1,file='../output/simulatedpop.dat')
		do i = 1, g_nsim, 1
			pop(pp)%id = i
			pop(pp)%age = g_agemin
			...
			call random_number(draw)
			pop(pp)%consumption = 100.0d3*draw
			if (pop(pp)%married) then
				pop(pp)%consumption = pop(pp)%consumption * 1.5d0
			end if	
			pp = pp + 1
			do a = 2, g_nages, 1
